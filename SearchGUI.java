    ...
    private void index() {
		...
		synchronized ( indexLock ) {
		    ...
		    for ( int i=0; i<dirNames.size(); i++ ) {
		    	System.out.println("Directory to be used: "+dirNames.get(i));
				File dokDir = new File( dirNames.get( i ));
				indexer.processFiles( dokDir );
		    }
		    if (verboseOn) {
		    	Iterator<String> indexIterator = indexer.index.getDictionary();
		    	System.out.println("Found tokens:");
		    	int nr = 0;
		    	while(indexIterator.hasNext() && (nr < 50)) {
		    		String msg = indexer.index.postingsString(indexIterator.next());
		    		System.out.println(msg);
		    		nr++;
		  	  	}
		  	}
		    System.out.println("NR OF DOCS INDEXED: "+indexer.index.docIDs.size());
		    indexer.index.cleanup();
		    resultWindow.setText( "\n  Done!" ); 
		}
    };
